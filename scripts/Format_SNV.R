...
snv = as.data.frame( fread( file.path(input_dir, "SNV.txt.gz") , stringsAsFactors=FALSE , sep="\t", fill = TRUE))

data = cbind( snv[ , c("Start_position" , "pair_id" , "Hugo_Symbol", "Variant_Classification" ) ] ,
...
data$Sample = sapply( data$Sample , function(x){ paste( unlist( strsplit( as.character( x ) , "-" , fixed=TRUE )) , collapse=".") })
