...
rownames(cna) = cna[ , 1] 
cna = cna[ , -( 1:3 ) ]
colnames(cna) = sapply( colnames(cna) , function(x){ paste( unlist( strsplit( as.character( x ) , "-" , fixed=TRUE )) , collapse=".") })

cna = unique( cna[ , colnames(cna) %in% case[ case$cna %in% 1 , ]$patient ] )

cna = cna[ - grep( "MIR1244-1" , rownames(cna) ) , ]
cna = cna[ - grep( "SNORA59A" , rownames(cna) ) , ]
